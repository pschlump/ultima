...
func (t *terminal) consumeANSIEscape(buf *bufio.Reader, ansiBuf *bytes.Buffer) (result readResult, err error) {
	...
	default:
		// Ultima patch: not a recognized Alt/ANSI continuation. Deliver the
		// ESC as its own keypress and leave the rune buffered for the next
		// read, so that in vi mode ESC exits insert mode and the following
		// key is interpreted as a normal-mode command (e.g. ESC k recalls
		// the previous history entry, like the up arrow). Upstream
		// swallowed both bytes here ("invalid, ignore"), which made
		// ESC+<key> unusable in vi mode.
		_ = buf.UnreadRune()
