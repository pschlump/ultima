...
func init() {
	jumpTable = [opCodeMax + 1]instFunc{
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_NEWTABLE
			...
			C := int(inst>>9) & 0x1ff //GETC
			v := newLTable(B, C)
			// Ultima memory accounting: approximate table cost.
			L.accountMem(64 + int64(B)*16 + int64(C)*48)
			// +inline-call reg.Set RA v
			return 0
		},
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_CONCAT
			...
			// Ultima memory accounting: charge the concatenated result.
			if cs, ok := v.(LString); ok {
				L.accountMem(int64(len(cs)))
			}
			// +inline-call reg.Set RA v
			return 0
		},
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_CLOSURE
			...
			// Ultima memory accounting: approximate closure cost.
			L.accountMem(int64(96 + 8*proto.NumUpvalues))
