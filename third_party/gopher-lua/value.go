...
type LTable struct {
	...
	// readonly marks the table as frozen (Ultima: the Redis-classic
	// script globals lockdown). Writes through the LState write paths
	// (setField/setFieldString, RawSet/RawSetInt) raise "Attempt to
	// modify a readonly table" unless the LState's bypass window is
	// open (SetReadonlyBypass). Bare *LTable method calls do not check
	// the flag.
	readonly bool
}

// SetReadonly marks/unmarks this table as frozen against script writes.
func (tb *LTable) SetReadonly(ro bool) { tb.readonly = ro }

// IsReadonly reports whether this table is frozen against script writes.
func (tb *LTable) IsReadonly() bool { return tb.readonly }
...
type LState struct {
	...
	// Ultima host extensions (the pure-Go host package).
	//
	// roBypass opens the readonly-table write window (the fork's
	// rt_globals_readonly(0) equivalent): while true, the readonly
	// checks in setField/setFieldString/RawSet/RawSetInt are skipped.
	// The host toggles it while staging host functions, constants, and
	// the per-run KEYS/ARGV globals. Not goroutine-safe — the owning
	// host.VM serializes all use.
	roBypass bool
	// memUsed/memLimit implement the approximate per-state allocation
	// budget (the fork's rt_set_memlimit equivalent). memUsed is a
	// MONOTONIC counter of bytes charged at the dominant allocation
	// points (string concat, string.rep, table creation, number-block
	// pages, closures, threads) — frees are not tracked, so it
	// overcounts a GC-ing state's live heap. When memLimit > 0 and a
	// charge crosses it, accountMem raises the pcall-catchable Lua
	// error "not enough memory" (Redis's text).
	memUsed  int64
	memLimit int64
}
