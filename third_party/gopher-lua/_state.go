...
func newLState(options Options) *LState {
	...
	ls.Env = ls.G.Global
	// Ultima: number-block page allocations count against the budget.
	al.account = ls.accountMem
	return ls
}
...
func (ls *LState) setField(obj LValue, key LValue, value LValue) {
	...
	for i := 0; i < MaxTableGetLoop; i++ {
		tb, istable := curobj.(*LTable)
		if istable {
			// Ultima: readonly tables (the globals lockdown) refuse ALL
			// writes before __newindex dispatch, with a position prefix —
			// Redis's luaV_settable patch.
			if tb.readonly && !ls.roBypass {
				ls.RaiseError("Attempt to modify a readonly table")
			}
...
func (ls *LState) setFieldString(obj LValue, key string, value LValue) {
	...
	for i := 0; i < MaxTableGetLoop; i++ {
		tb, istable := curobj.(*LTable)
		if istable {
			// Ultima: readonly check before __newindex dispatch (see
			// setField).
			if tb.readonly && !ls.roBypass {
				ls.RaiseError("Attempt to modify a readonly table")
			}
...
func (ls *LState) NewTable() *LTable {
	ls.accountMem(64 + 16*defaultArrayCap + 48*defaultHashCap)
	return newLTable(defaultArrayCap, defaultHashCap)
}

func (ls *LState) CreateTable(acap, hcap int) *LTable {
	ls.accountMem(64 + int64(acap)*16 + int64(hcap)*48)
...
func (ls *LState) NewThread() (*LState, context.CancelFunc) {
	ls.accountMem(4096) // Ultima memory accounting: approximate thread cost
...
func (ls *LState) RawSet(tb *LTable, key LValue, value LValue) {
	...
	// Ultima: raw writes to a readonly table raise the BARE string (no
	// position prefix — the raise runs inside the base library function,
	// exactly like Redis's lua_rawset patch), catchable by pcall.
	if tb.readonly && !ls.roBypass {
		panic(&ApiError{Type: ApiErrorRun, Object: LString("Attempt to modify a readonly table")})
	}
	tb.RawSet(key, value)
}
	...
	defer func() {
		...
		if rcv != nil {
			...
			if errfunc != nil {
				...
				ls.Call(1, 1)
				herr := newApiError(ApiErrorError, ls.Get(-1))
				// Ultima: keep the panic class visible through the error
				// handler so the host can distinguish a real Go panic
				// (backend bug → TrapError) from a script-raised value.
				if err.(*ApiError).Type == ApiErrorPanic {
					herr.Type = ApiErrorPanic
					herr.Cause = err.(*ApiError).Cause
				}
				err = herr
...
/* Ultima host extensions {{{ */

// SetReadonlyBypass opens (true) or closes (false) the readonly-table
// write window. The host package opens it while staging host functions,
// constants, and the per-run KEYS/ARGV globals — mirroring the wasm
// runtime's rt_globals_readonly toggle. Not goroutine-safe; the owning
// host.VM serializes all use.
func (ls *LState) SetReadonlyBypass(on bool) { ls.roBypass = on }

// SetMemoryLimit sets the per-state approximate allocation budget in
// bytes (0 = unlimited). Crossing the budget raises the pcall-catchable
// Lua error "not enough memory" at the next accounted allocation point.
func (ls *LState) SetMemoryLimit(n int64) { ls.memLimit = n }

// MemoryUsed returns the monotonic approximate allocation counter (see
// the memUsed field documentation): it overcounts the live heap because
// frees are never subtracted.
func (ls *LState) MemoryUsed() int64 { return ls.memUsed }

// accountMem charges n bytes to the state's approximate allocation
// counter, raising "not enough memory" (Redis's MEMERRMSG text) when the
// budget is exceeded. The raise is an ordinary Lua error — pcall
// catches it.
func (ls *LState) accountMem(n int64) {
	ls.memUsed += n
	if ls.memLimit > 0 && ls.memUsed > ls.memLimit {
		ls.RaiseError("not enough memory")
	}
}
