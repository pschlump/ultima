...
|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------|
| **M0** | Skeleton: config, logging, three listeners (stub), Makefile, lint, CI-local                                                                                                                                                                                                                                                                                                                                                  | **M0 done** `make build test lint` green; PING on all three surfaces                                        |
| **M1** | Shard engine + RESP front-end + P0 commands; redcon fork w/ RESP3                                                                                                                                                                                                                                                                                                                                                            | **M1 done** redis-cli fully works for P0; differential harness green on P0; first benchmark report vs Redis |
| **M2** | P1 collections (pluto zset range/rank + quicklist, §5.3 #1/#10)                                                                                                                                                                                                                                                                                                                                                              | **M2 done** differential green on H/L/S/Z                                                                   |
| **M3** | P2 transactions + classic pub/sub + blocking list/zset ops (SSUBSCRIBE → M8; keyspace notifications → M5)                                                                                                                                                                                                                                                                                                                    | **M3 done** MULTI/EXEC + WATCH stress green; redis-benchmark pub/sub                                        |
| **M4** | gRPC + WS front-ends; proto IDL stable                                                                                                                                                                                                                                                                                                                                                                                       | **M4 done** go + ts clients round-trip; parity with RESP replies                                            |
| **M5** | Expiry hardening, eviction, persistence (snapshot + AOF), keyspace notifications (notify-keyspace-events; deferred from P2)                                                                                                                                                                                                                                                                                                  | **M5 done** crash-recovery tests; maxmemory soak                                                            |
| **M6** | Auth system (§9) + HTTP API + web UI v1. **M6a done** (accounts, Ed25519 JWT, TOTP, auth on HTTP/gRPC/WS — `docs/m6-detailed-plan.md`); **M6b done** (resumable WS sessions, §9.4 — `lib/wssession`); **M6c done** (OpenAPI management API, §10.1 — `api/openapi.yaml` + `lib/httpapi`); **M6d done** (web UI v1, §10.2 — `web/` embedded SPA, console over /ws/v1 with session recovery, MONITOR command, WS origin policy) | login/refresh/TOTP green; multi-admin accounts; WS recovery tests pass; dashboard + console live            |
| **M7** | Client libraries (§11) + example applications. **Done** — `clients/go/ultima` (RESP/gRPC/WS/REST umbrella client, §9.4 WS recovery) + CLIs `ultima-cli`/`ultima-ws-cli`/`ultima-grpc-cli`; `clients/typescript` `@ultima/client` (web UI refactored onto it) + `clients/javascript` ESM/CJS dist; `examples/leaderboard` + `examples/chat` — `docs/m7-detailed-plan.md` | Go + TS + JS packages build; CLIs run on the Go client; leaderboard + chat examples run end-to-end          |
| **M8** | P3/P4 parity tail (streams, Lua-lite, bitfield, geo, PF\*), sharded pub/sub SSUBSCRIBE/SPUBLISH (deferred from P2). Tail landed earlier; **Lua scripting done** — EVAL/EVALSHA/EVAL_RO/EVALSHA_RO + SCRIPT on the gopher-lua wasm engine (D12, `lib/scripting`; `docs/m8-detailed-plan.md`, probes in `docs/Redis-Errors.md`)                                                                                                                                                                                                                                                                                                           | differential green on covered tail; **scripting differential-green** (`tests/differential/scripts_m8.go`), AOF-effects crash recovery (`tests/m8_aof_test.go`)                                                                          |
| **M9** | Superset features (§12) + performance campaign — plan in `docs/m9-detailed-plan.md` (perf first, then §12.2–§12.5 features; §12.6 warm tier deferred)                                                                                                                                                                                                                                                                                                                                                                               | ≥4× Redis on target workload; final report                                                                  |
